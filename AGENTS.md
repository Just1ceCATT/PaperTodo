...
- **只要本次任务产生用户可感知差异，无论通过 PR、手动合并、直接提交主线还是多提交串联实现，任务结束前都必须对照 `Unreleased`；需要时在同一任务更新，不把检查绑定在“准备合并”动作上。**
- 做版本整理、发布、整合收尾，或发现目标主线在本次任务之外已经前进时，必须以**目标分支实际提交历史**反查最近用户可感知变化；PR 列表和当前任务提交都不能作为唯一来源。
