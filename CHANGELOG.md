...
- **边缘胶囊实时预览**：鼠标悬停在边缘胶囊上即可展开轻量、可交互的预览卡片，无需打开完整纸片即可即时浏览内容。
  ...
  - **笔记预览**：跟随 Markdown 渲染设置，关闭时显示原文，基础提供轻量高亮，增强淡化语法并显示列表圆点、分割线，编号和任务状态保持清晰，完全渲染展示紧凑排版；支持标题、强调、删除线、代码块、列表与图片占位，避免普通笔记因空行或长段落过早省略。
...
**待办与 Markdown**
- **内置全文搜索**：`Ctrl+F` 可跨全部待办和 Markdown 笔记查找，包含已完成的待办，显示当前纸片与全局命中计数；`Enter` / `Shift+Enter` 或箭头按钮可循环跳转并定位到具体待办项或笔记文字，自动显示隐藏纸片、展开胶囊并接续搜索。搜索框唤出后可直接输入，从纸片内展开并允许超出边框；可通过靠右的圆角竖条拖动柄、计数区或空白区域移动，采用无阴影样式，上下箭头和关闭按钮使用矢量图标。
- **完全渲染升级为所见即所得块级编辑**：笔记选择「完全渲染」档，编辑时也按标题、列表、引用、代码块、图片与行内样式等最终排版直接显示，并隐藏多数 Markdown 标记
标题、加粗、链接等文字紧凑重排；任务框、无序列表和引用使用稳定占位，减少勾选、编辑切换与引用续行时的横向跳动；有序列表编号始终直接显示原文字，避免编辑切换时闪动；光标所在块会显示其源码标记，失焦回到整篇只读渲染。
...
- **失焦形态与置顶自定义**：支持纸片失焦时仅隐藏顶栏按钮，或将标题栏区域整体淡出，完全隐藏后可点击到后面的窗口，正文位置和窗口尺寸保持不变；支持让贴边胶囊与主胶囊不再强制保持系统最前台置顶。
