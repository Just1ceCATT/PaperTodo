...
- **完全渲染升级为所见即所得块级编辑**：笔记选择「完全渲染」档后，编辑时也按标题、列表、引用、代码块、图片与行内样式等最终排版直接显示，并隐藏多数 Markdown 标记——这些标记不再占宽度，标题、加粗、链接等文字按最终排版紧凑重排；光标所在块会显示其源码标记，便于直接调整标题级别、列表、引用与代码块。编辑器始终是同一个文本控件，失焦回到整篇只读渲染，光标、选区、撤销栈、复制粘贴保持一致；渲染、标记隐藏与显灵只改变显示，不会自动改写 Markdown 原文。控制符显灵可使用「Markdown 渲染动画」短淡入，该设置仅在选择「完全渲染」时显示。
...
- **多纸片管理**：创建和管理多张独立纸片，自动记录位置、大小、置顶状态与内容。
