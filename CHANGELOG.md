...
**待办与 Markdown**
- **内置全文搜索**：`Ctrl+F` 可跨全部待办和 Markdown 笔记查找，包含已完成的待办，显示当前纸片与全局命中计数；`Enter` / `Shift+Enter` 或箭头按钮可循环跳转并定位到具体待办项或笔记文字，自动显示隐藏纸片、展开胶囊并接续搜索。搜索框唤出后可直接输入，从纸片内展开并允许超出边框；可通过右侧拖动柄、计数区或空白区域移动，采用无阴影样式。
...
- 修复开启“记住展开位置”时，纸片与边缘胶囊位于不同屏幕，收起后再次展开会被拉回胶囊所在屏幕的问题。
