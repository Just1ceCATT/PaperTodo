...
- **完全渲染升级为所见即所得块级编辑**：笔记选择「完全渲染」档后，编辑时也按标题、列表、引用、代码块、图片与行内样式等最终排版直接显示，并隐藏多数 Markdown 标记——这些标记不再占宽度，标题、加粗、链接等文字按最终排版紧凑重排；光标所在块会显示其源码标记，便于直接调整标题级别、列表、引用与代码块。编辑器始终是同一个文本控件，失焦回到整篇只读渲染，光标、选区、撤销栈、复制粘贴保持一致。
